...
Notation " x ↓ n ∈ Γ " := (item x Γ n) (at level 80, no associativity) : DN_scope.
...
(* Env *)
Definition dom Γ := @List.length Term Γ.
...
(* handfull lemmas about ins and lift *)
Lemma ins_size : forall Γ d n Γ1 Γ2, ins_in_env Γ d n Γ1 Γ2 -> dom Γ2 = S(dom Γ1).
induction 1.
simpl ; reflexivity.
simpl; rewrite IHins_in_env; reflexivity.
Qed.
...
Notation " t ↓ n ⊂ Γ " := (item_lift t Γ n) (at level 80, no associativity): DN_scope.
...
Lemma sub_size : forall Γ t T n Δ Δ', sub_in_env Γ t T n Δ Δ' ->
 dom Δ = S (dom Δ').
intros.
induction H; simpl; intuition.
Qed.

Lemma sub_out : forall Δ n, dom Δ = n -> 
  forall Γ t T m Δ', sub_in_env Γ t T m Δ Δ' -> m >= n -> False.
intros Δ; induction Δ; intros.
inversion H0.
simpl in H.
inversion H0; subst; clear H0.
assert (S (dom Δ') = 0) by intuition.
discriminate.
eapply IHΔ.
reflexivity.
apply H6.
