module coq-maximal-fpc.
/* start */
storeL_jc  (max {{lp:N}} ({{Cd lp:N lp:A}})) (max {{S lp:N}} A) (ix N).
decideL_je    (max {{N}}  ({{Cd N A}})) (max {{S N}} A) (ix N).
decideR_je  (max {{N}} ({{C1 A}}))   (max {{N}} A).
storeR_jc   (max {{N}} ({{C1 A}}))   (max {{N}} A).
releaseL_je (max {{N}} ({{C1 A}}))   (max {{N}} A).
releaseR_je (max {{N}} ({{C1 A}}))   (max {{N}} A).
initialL_je (max _N {{C0}}).
initialR_je (max _N ({{Cx I}})) (ix I).
